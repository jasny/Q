...
class Config_JsonTest extends Config_MainTest
{
	/**
    ...
    {
    	$config = new Config_Json(array('path'=>$this->getPath() . '/test.json'));
    	$this->setgetTest($config);
    }

    public function testConfigSubgrpFile()
    {
    	$config = new Config_Json(array('path'=>$this->getPath() . '/test-subgrp.json'));
    ...
    {
    	$config = new Config_Json(array('path'=>$this->getPath() . '/test-php.json', 'php'=>true, 'parameters'=>array('good'=>true)));
    	$this->setgetTest($config);
    }

    public function testConfigPHPFile_Alt()
    {
    	$config = new Config_Json(array('path'=>$this->getPath() . '/test-php.json', 'php'=>true));
    	$this->assertEquals(array('q'=>'Not good', 'b'=>27), $config->get('grp1'));
    }
    
    public function testConfigDir()
    {
    	$config = new Config_Json(array('path'=>$this->getPath() . '/test'));
