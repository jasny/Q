...
//    	if (!extension_loaded('syck')) $this->markTestSkipped("Extension 'syck' is not available.");
    ...
    {
    	$config = new Config_Yaml(array('path'=>$this->getPath() . '/test.yaml'));
    	$this->setgetTest($config);
    }

    public function testConfigSubgrpFile()
    {
    	$config = new Config_Yaml(array('path'=>$this->getPath() . '/test-subgrp.yaml'));
    ...
    {
/*
    	$config = new Config_Yaml(array('path'=>$this->getPath() . '/test-php.yaml', 'php'=>true, 'parameters'=>array('good'=>true)));
    	$this->setgetTest($config);
*/
    }
    ...
    {
/*
    	$config = new Config_Yaml(array('path'=>$this->getPath() . '/test-php.yaml', 'php'=>true));
...
*/
    ...
    {
    	$config = new Config_Yaml(array('path'=>$this->getPath() . '/test'));
    	
var_dump($config->get('grp1')); exit;    	
