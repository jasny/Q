...
/**
 * Testing basic features of DB_MySQL.
 * This includes querying, quoting and basic parsing.
 */
class DB_MySQL_BasicTest extends \PHPUnit_Framework_TestCase
{
    /**
	 * Run test from php
	 */
     ...
     * More parsing tests are done in sqlSplitter test case
     */
    ...
    {
    	$result = $this->conn->query("SELECT * FROM test WHERE status='ACTIVE'");
    	$this->assertEquals(array(array(1, 'one', 'first row', 'ACTIVE'), array(2, 'two', 'next row', 'ACTIVE')), $result->getAll(DB::FETCH_ORDERED));
    }
    ...
    {
    	$result = $this->conn->query("SELECT * FROM test WHERE status='ACTIVE'");
    	$this->assertEquals(array(array('id'=>1, 'key'=>'one', 'title'=>'first row', 'status'=>'ACTIVE'), array('id'=>2, 'key'=>'two', 'title'=>'next row', 'status'=>'ACTIVE')), $result->getAll(DB::FETCH_ASSOC));
    }
    ...
    {
    	$result = $this->conn->query("SELECT * FROM test WHERE status='ACTIVE'");
    	$this->assertEquals(array(array(1, 'one', 'first row', 'ACTIVE', 'id'=>1, 'key'=>'one', 'title'=>'first row', 'status'=>'ACTIVE'), array(2, 'two', 'next row', 'ACTIVE', 'id'=>2, 'key'=>'two', 'title'=>'next row', 'status'=>'ACTIVE')), $result->getAll(DB::FETCH_FULLARRAY));
    }
    ...
    {
    	$result = $this->conn->query("SELECT title FROM test WHERE status='ACTIVE'");
    	$this->assertEquals(array('first row', 'next row'), $result->getAll(DB::FETCH_VALUE));
    }
    ...
    /**
     * Test DB_MySQL_Result->seekRows() searching on column name
     */
    ...
    /**
     * Test DB_MySQL_Result->seekRows() fetching assoc row
     */
    ...
    /**
     * Test DB_MySQL_Result->seekRows() fetching values
     */
    ...
    /**
     * Test inserting multiple rows using conn->store() with DB::arg('values', rows)
     */
    public function testStore_InsertMultipleArgs()
    {
    	$this->conn->store('test', array('id', 'key', 'title', 'status'), DB::arg('values', array(array(null, 'new1', 'new row ONE', 'NEW'), array(null, 'new2', 'new row TWO', 'NEW'))));
