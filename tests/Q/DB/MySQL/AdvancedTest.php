 ...
 */
class DB_MySQL_AdvancedTest extends \PHPUnit_Framework_TestCase
{
	/**
	 * Run test from php
	 */
	...
	public function createTestSchema()
	{
		try {
			...
			$this->conn->query("CREATE TABLE `junction` (`idTest` INTEGER UNSIGNED NOT NULL, `child_id` INTEGER UNSIGNED NOT NULL, PRIMARY KEY(idTest, child_id))");
	...
	{
		$this->assertEquals("idTest", $this->conn->table('child')->getFieldProperty('#role:parentkey', 'name'), "child.#role:parentkey");
	}
	...
	public function testTableGetProperties_Junction()
	{
		...
		$this->assertEquals('idTest', $this->conn->table('junction')->getFieldProperty('#role:parentkey', 'name'), '#role:parentkey.name');
	...
	{	
		...
		$this->assertEquals('idTest', $field['name']);
	}
	...
	{
		$result = $this->conn->query("SELECT child.*, test.id as parent_id, test.title as parent_desc FROM test LEFT JOIN child ON test.id = child.idTest");
		$this->assertEquals($result->numFields(), sizeof($result->getFields()), "Incorrect number of fields:");
	...
	{
		$record = $this->conn->prepare("SELECT child.*, test.id as parent_id, test.title as parent_desc FROM test LEFT JOIN child ON test.id = child.idTest")->load(1);
		$this->assertEquals('child 1', $record->subtitle);
	...
	{
		$record = $this->conn->prepare("SELECT child.*, test.id as parent_id, test.title as parent_desc FROM test LEFT JOIN child ON test.id = child.idTest")->newRecord();
		...
		$this->assertNotNull($record->parent_id, $record->idTest, "parent_id != idTest");
		
		$id = $record->getId();
		$record = $this->conn->prepare("SELECT child.*, test.id as parent_id, test.title as parent_desc FROM test LEFT JOIN child ON test.id = child.idTest")->load($id);
	...
	{
	    $result = $this->conn->query("SELECT *, ROWS(SELECT * FROM child CASCADE ON child.idTest = test.id) AS children FROM test");
