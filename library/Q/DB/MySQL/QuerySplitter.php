...
require_once 'Q/DB/QuerySplitter.php';
...
class DB_MySQL_QuerySplitter implements DB_QuerySplitter
{
	...
	public static function parse($statement, $args)
	{
		...
		$i = 0;
		$statement = preg_replace('/`[^`]*+`|"(?:[^"\\\\]++|\\\\.)*+"|\'(?:[^\'\\\\]++|\\\\.)*+\'|(\?\!?)|:(\!)?(\w++)/e', !isset($args) ? "'$1$3' ? NULL : str_replace('\\\"', '\"', '\$0')" : "'\$3' && array_key_exists('\$3', \$args) ? ('\$2' ? \$args[':\$3'] : Q\DB_MySQL_QuerySplitter::quote(\$args[':\$3'])) : ('\$1' ? ('\$1' === '?!' ? \$args[\$i++] : Q\DB_MySQL_QuerySplitter::quote(\$args[\$i++])) : str_replace('\\\"', '\"', '\$0'))", $statement);
	...
	static protected function splitInsertQuery($sql)
	{
		...
		{
		 	throw new DB_Exception("Unable to split INSERT/REPLACE query, invalid syntax:\n" . $sql);
		}
	 ...
	 * @return array
     *
     * @todo Implement DB_QuerySplitter_MySQL::splitJoin()
	 */
